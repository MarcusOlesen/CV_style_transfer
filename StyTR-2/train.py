...
def adjust_learning_rate(optimizer, iteration_count):
    ...
    lr = 1e-4 / (1.0 + (args.lr_decay * 10) * max(0, iteration_count - 1e4))
...
def warmup_learning_rate(optimizer, iteration_count):
    ...
    # print(lr)
    warmup_iters = 10000
    factor = min(1.0, iteration_count / warmup_iters)
    lr = args.lr * 0.1 * (1.0 + 3e-4 * iteration_count) * factor
    for param_group in optimizer.param_groups:
         param_group['lr'] = lr
...
parser.add_argument('--lr_decay', type=float, default=1e-4)#1e-5
parser.add_argument('--max_iter', type=int, default=160000)
parser.add_argument('--batch_size', type=int, default=2)#8
parser.add_argument('--style_weight', type=float, default=1.0)#10.0
parser.add_argument('--content_weight', type=float, default=1.0)#7.0
