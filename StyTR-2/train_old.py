...
for i in tqdm(range(args.max_iter)):
   ...
   # print(f"Model is on: {next(network.parameters()).device}")

    out, loss_c, loss_s,l_identity1, l_identity2 = network(content_images, style_images)

    if i % 100 == 0:
        print(f"Saving at iteration {i}")
        print(f"content: {content_images.shape}, style: {style_images.shape}, out: {out.shape}")
        print(f"Output min/max: {out.min().item()}, {out.max().item()}")
